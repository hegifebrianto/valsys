...
class DX_Auth
{
	...
	*/
	...
	{
		// Get role data
		$role_data = $this->_get_role_data($data->USER_ID);	
        $grup_data = $this->_get_grup_data($data->USER_ID);
	...
	function _get_grup_data($user)
	{
	    // Load models
		    $this->ci->load->model('dx_auth/roles_group', 'group');
			$q = $this->ci->group->get_grup_by_user($user);			
	...
	// check app privilege module ext by grup	
	function _get_app_privilege_module_ext_by_grup($ext)
	{
	    /* $grup_result = array(); */
		...
		if ($this->is_logged_in())
		{
		    $a = array(); 
			// Load models
		    $this->ci->load->model('dx_auth/app_privilege_module_ext', 'privilege_ext');
			$this->ci->load->model('dx_auth/app_privilege_module', 'm_modules');
			$controller =$this->ci->uri->segment(1)  .'-'.$this->ci->uri->segment(2)  ; 
			$module = $this->ci->m_modules->get_module_id($controller);	
			//$module = $this->ci->input->get('id');
				
			// grup data user default
			$grup_data = $this->get_grup_data();
			
			if ($grup_data) {
				foreach ($grup_data as $key=>$val)
				{
					$pri_by_grup = $this->ci->privilege_ext->get_pri_grup($ext,$module,$val);	
					
					if($pri_by_grup->num_rows() > 0 )
					{
						foreach  ($pri_by_grup->result_array() as $value)
						{

							$a[] = $value['PRV_DYN_PARAMS2']; 							
						}
					}
					else
					{
            ...
            //var_dump($a);			
			/* jika ada user berhak akses modul 
			   dalam salah satu grup kembalikan nilai true 
	...
	// check app privilege module ext by user	
	function _get_app_privilege_module_ext_by_user($ext)
	{
	    // First check if user already logged in or not
		if ($this->is_logged_in())
		{
			...
			$pri_by_user = $this->ci->privilege_ext->get_pri_by_user($this->get_user_id(),$ext,$module);
                
            if ($pri_by_user->num_rows() > 0)
            {
                foreach  ($pri_by_user->result_array() as $value)
				{
 				...
 				/* jika ada user berhak akses modul khusus 
			       dalam salah satu grup kembalikan nilai true 
	...
	{
	    // Load models
		$this->ci->load->model('dx_auth/app_privilege_module_ext', 'privilege_module');
		$module = $this->ci->input->get('id');
		$grup_data = $this->get_grup_data();
		foreach ($grup_data as $key=>$val)
		{
			$pri_by_grup = $this->ci->privilege_module->get_app_pri_module($val,$module,$this->get_user_id());	
			foreach  ($pri_by_grup->result_array() as $value)
			{
	...
	function is_allow_module()
	{
	    if ($this->_check_module())
            return TRUE;
        else
            return FALSE; 		
	}
	
	function is_allow_ext($ext)
	
	{
	    $result = FALSE;
		
		$by_grup = $this->_get_app_privilege_module_ext_by_grup(strtoupper($ext));
		$by_user = $this->_get_app_privilege_module_ext_by_user(strtoupper($ext));
	...
	*/
	...
	function deny_access($uri = 'deny')
	{
		...
		{
			redirect($this->ci->config->item('DX_index_uri'), 'location');
		}		
		else
		{
	...
	function get_grup_data()
	{
		//var_dump($this->ci->session->userdata('grup_data'));
		/* if($this->ci->session->userdata('grup_data') === FALSE)
		{
		    $a = array(); 
			return $a;
		}
		{ */
		   ...
		   return $this->ci->session->userdata('grup_data');
		//}	
	}
	...
	function get_user_desc()
	{
		return $this->ci->session->userdata('user_descr');
	}
	...
	function login($login, $password, $remember = TRUE)
	{
		...
		if ( ! empty($login) AND ! empty($password))
		{
			...
			if ($this->ci->config->item('DX_login_using_username') AND $this->ci->config->item('DX_login_using_email'))
			{
				...
				$get_user_function = 'get_user_by_objectguid';
			}
			...
			if ($return = $this ->ci->auth_ad->search_ad($login, array(
                    'dn',
                    'cn',
                    'objectguid'
             ))) 
			 {
					$ObjectGUID = $this->GUIDtoStr($return['objectguid'][0]);
					//var_dump($get_user_function );exit;
					if ($query = $this->ci->users->$get_user_function($ObjectGUID ) AND $query->num_rows() == 1)
					{
						// Get user record
						$row = $query->row();
						...
						if ($row->IS_LOCKED > 0)
						{
							...
							$this->_banned = TRUE;					
							// Set ban reason
							$this->_ban_reason = $row->IS_LOCKED;
						}
						// If it's not a banned user then try to login
						else
						{					
							//$password = md5($password);
							//$stored_md5 = $row->PASSWORD;
							//var_dump($password,$stored_hash);exit;
							// Is password matched with hash in database ?
							//if ($password === $stored_md5)
							if ($entries = $this ->ci->auth_ad-> bind_as_user($login, $password))
							{
								// Log in user 
								...
								// Trigger event
								$this->user_logged_in($row->user_id);
	...
	{	
			...
			$is_allowed		= $this->ci->m_modules->get_access_allowed($controller ,$this->get_user_id());
