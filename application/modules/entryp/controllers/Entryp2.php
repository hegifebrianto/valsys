...
class Entryp2 extends CI_Controller {

	function __construct(){
		parent::__construct();
		$this->load->model("m_entryp2");
	...
	public function add_datadth(){
		...
		$this->m_entryp2->add_data_dth($idp, $category,$sub_category);
	...
	{   
	 ...
	 function getProspectn(){
	 	var_dump($_GET); exit;
			...
			var_dump($q );exit;
			$this->m_entryp2->show_prospectn($q);
			}	 
	  }
	...
	{	
		...
		//DROPDOWN PROVINCE
		$data['provinsi']=$this->m_entryp2->provinsi();
		
		$data['allData'] = $this->m_entryp2->dataep_r();
		$data['allDataW'] = $this->m_entryp2->dataep_rw();
	...
	public function h_detail($id){
		...
		//DROPDOWN PROMO
		$data['promotion'] = $this->m_entryp2->promotion();
		
		//DROPDOWN PRODUCT
		$data['produk'] = $this->m_entryp2->product();
		$data['admin_reject'] = $this->m_entryp2->admin_reject();
		$data['admin_approve'] = $this->m_entryp2->admin_approve();

		//$data['allData'] = $this->m_entryp2->getAllData();
		$data['ep_dtd'] = $this->m_entryp2->getDataep($id);
    ...
    public function get_branch() {
        $query = $this->m_entryp2->get_allbranch(); //query model
    ...
    public function Update_EP_BFP_DB(){
		...
		$data 					= curl_exec($ch);
		curl_close($ch);
		$this->m_entryp2->updateDataEP_BFP($statusep, $prospectid,$noteadmin);
	...
	{	
	   ...
	   //header('Content-Type: application/json');
		...
		/* //DROPDOWN PROVINCE
		$data['provinsi']=$this->m_entryp2->provinsi();
		$data['branch'] = $this->m_entryp2->branch();
		//DROPDOWN PROMO
		$data['promotion'] = $this->m_entryp2->promotion();
		$data['salesMS'] = $this->m_entryp2->salesMS();

		//DROPDOWN BRAND
		$data['brand'] = $this->m_entryp2->brand();

		//DROPDOWN SALES CHANNEL
		$data['sales_channel'] = $this->m_entryp2->sales_channel();
		
		//DROPDOWN PRODUCT
		$data['produk'] = $this->m_entryp2->product();
		$data['prospect_type'] = $this->m_entryp2->prospect_type(); */
		
		$data['allData'] = $this->m_entryp2->getAllData();
		$data['allDataHW'] = $this->m_entryp2->getDataHW();
		
			//$data['id_combo']=$id_combo;
	//	$data['prospect_type'] = $this->entryp->prospect_type();

		$this->load->view('v_entryp2',$data);
	...
	{	
		...
		//DROPDOWN PROVINCE
		$data['provinsi']=$this->m_entryp2->provinsi();
		$data['allData'] = $this->m_entryp2->dataep_reg_dth();
	...
	public function updateEP_Reg_Dth($id){
		...
		$data['category_dth']=$this->m_entryp2->get_categorydth();
		//$data['subcategory_dth']=$this->m_entryp2->get_subcategorydth();
		//DROPDOWN PROVINCE
		$data['provinsi']=$this->m_entryp2->provinsi();
		...
		//DROPDOWN PRODUCT
		$data['produk'] = $this->m_entryp2->product();
		$data['disposisi'] = $this->m_entryp2->disposisi();
	...
	public function updateEP_Reg_Dth_db(){
		...
		$data 					= curl_exec($ch);
		curl_close($ch);
		$this->m_entryp2->updateDataEP_Reg_dth($statusep, $prospectid,$dthnote);
		redirect('entryp/Entryp/reg_admin');
	...
	public function updateEP_DTD($id){
		...
		$data['admin_reject'] = $this->m_entryp2->admin_reject();
		$data['admin_approve'] = $this->m_entryp2->admin_approve();
		$data['light_verification'] = $this->m_entryp2->get_light_verification();
	...
	public function bfp($id){
		...
		//$data['allData'] = $this->m_entryp2->getAllData();
		$data['ep_dtd'] = $this->m_entryp2->getDataep($id);
		$data['ep_dtd_btp'] = $this->m_entryp2->getDatabtp($id);
		$data['ep_dtd_btpe'] = $this->m_entryp2->getDatabtpe($id);
	...
	public function Update_EP_DTD_DB(){
		...
		$this->m_entryp2->updateDataEP_DTD($statusep,$prospectid,$actid,$duplicate_value_name,$duplicate_value_phone,$duplicate_stat,$light,$Type1,$Type2,$Type3,$Type4,$Approve1,$Approve2,$Approve3,$Approve4,$Reject1,$Reject2,$Reject3,$Reject4);
		/*$this->m_entryp2->updateDataEP_DTD_INSERT($prospectid,$Type1,$Type2,$Type3,$Type4,$Approve1,$Approve2,$Approve3,$Approve4,$Reject1,$Reject2,$Reject3,$Reject4);*/
		redirect('entryp/Entryp/reg_admin');
...
//$nmfile = "file_".time(); //nama file saya beri nama langsung dan diikuti fungsi time
        ...
        $this->upload->initialize($config);
            ...
            if ($this->upload->do_upload('userfile'))
            {
                ...
                $this->m_entryp2->DTDaddDataProspect($prospect_type,$prospect_nbr,$first_name,$home_phone,$mobile_phone,$email,$religion,$hardware_status,$id_number,$gender,$date_of_birth,$street,$province,$city,$district,$sub_district,$zip_code,$order_date,$payment_method, $data);
...
*/
		...
		/*$this->m_entryp2->DTDaddDataProspect($prospect,$address,$contract,$additional);*/
		/*$this->m_entryp2->DTDaddDataNotesP($data1);*/
/*		$this->m_entryp2->DTDaddDataAddress($data2);*/
	...
	/*public function DTDaddData(){
		...
		$this->m_entryp2->DTDaddDataAddress($data2);
		$this->m_entryp2->DTDaddDataProspect($data);
		$this->m_entryp2->DTDaddDataNotesP($data1);
		redirect('Entryp');
...
//EndofDTD
	...
	{   
	   $data = $this->db->get('MASTER_EMPLOYEE')->result();
	   $arr_data = array();
	   $i=0;
	   foreach($data as $r){
	...
	{
		$data = $this->db->get('MASTER_PROSPECT_UPLOAD')->result();
	   	$arr_data = array();
	   	$i=0;
	   foreach($data as $r){
	...
	public function angular_UploadData()
	{
    	...
    	$year = $date_folder[0];
    	$month = $date_folder[1];
    	$final_path = $mypath.$year.'/'.$month.'/'.$name;

    	

 		if (!file_exists($mypath.$year)) {
    		mkdir($mypath.$year, 0777, true);
    		//chown($mypath.$date_folder[0], "subsdev");
    		chdir('/'.$year);


    		if (!file_exists($mypath.$year.'/'.$month)) {
	    		mkdir($mypath.$year.'/'.$month, 0777, true);
	    		chown($mypath.$year.'/'.$month, "subsdev");
	    		print_r("Directory 2 successfully created");
        ...
        else
        {
        	...
        	{
	    		mkdir($mypath.$year.'/'.$month, 0777, true);
	    		chown($mypath.$years.'/'.$month, "subsdev");
        ...
        //upload database
    	$this->m_entryp2->SaveDataUploaded($name,$file_category,$type,$final_path,$prospect_nbr,$uploadedby);
	}
	...
	function ambil_data(){
		...
		if($modul=="kota"){
		echo $this->m_entryp2->kota($id);
		
		}
		else if($modul=="kecamatan"){
		echo $this->m_entryp2->kecamatan($id);

		}
		else if($modul=="kelurahan"){
		echo $this->m_entryp2->kelurahan($id);
		}
		else if($modul=="alacarte")
		{
		echo $this->m_entryp2->alacarte($id);
		}
		else if($modul=="subc")
		{
		echo $this->m_entryp2->get_subcategorydth($id);
		}
	}
	...
	function getDataHW(){  
		$result  = $this->m_entryp2->getDataHW();		 
		return  json_encode($result);
	} 
	
	function getTemplate(){
		//$id 				= strtoupper(trim($this->input->post('id_combo')));
		$result  		= $this->m_entryp2->getTemplate();
		return  json_encode($result);
	}
	
		function getDataHW2(){  
		$id_combo 				= $this->input->post('id_combo'); 
		$result  = $this->m_entryp2->getDataHW2($id_combo);	
			return  json_encode($result);
...
//	return  json_encode($result);
		...
		$result  = $this->m_entryp2->getDataHW3();		 
		return  json_encode($result);
	}
