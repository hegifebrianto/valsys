...
class M_entryp2 extends CI_Model{
	 //var $tabel = 'PROSPECT';
...
/* 	function getDataHW(){
		$this->db->from("PROSPECT_ENTRY_HARDWARE");
		return $this->db->get();
	}  */
		function getDataHW(){
	...
	function getDataHW2($id_combo){
		 	...
		 	$sql =  "SELECT * FROM INV_V_PACKAGE_TEMPLATE  WHERE PACKAGE_TYPE_ID = $id_combo";
			$query=  $this->db->query($sql);
			$rows  = $query->result_array();		
			$result['status'] = 1;
	...
	/* function getDataHW($id){
		...
		$sql    = "SELECT * FROM INV_V_PACKAGE_TEMPLATE  WHERE PACKAGE_TYPE_ID =  $id ";
		$result = $this->db->query($sql)->result_array();
		return $result;
	} */
...
*/	
	...
	function dataep_rw(){
		//$this->db->from("PROSPECT");
		//return $this->db->get();
		  $sql="SELECT  * FROM PROSPECT where NOT statusep='NEW' OR statusep='INPROGRESS'";
		//$query=  $this->db->query($sql);
		  return $this->db->query($sql);
	}
